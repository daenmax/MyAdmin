...
import cn.daenx.framework.quartz.handle.QuartzDisallowConcurrentExecution;
import cn.daenx.framework.quartz.handle.QuartzJobExecution;
...
public class ScheduleUtils {
    ...
    public static boolean whiteList(String invokeTarget) {
        ...
        if (count > 1) {
            return StringUtils.containsAnyIgnoreCase(invokeTarget, QuartzConstant.JOB_WHITELIST_STR);
        }
        Object obj = SpringUtil.getBean(StringUtils.split(invokeTarget, ".")[0]);
        String beanPackageName = obj.getClass().getPackage().getName();
        return StringUtils.containsAnyIgnoreCase(beanPackageName, QuartzConstant.JOB_WHITELIST_STR)
                && !StringUtils.containsAnyIgnoreCase(beanPackageName, QuartzConstant.JOB_ERROR_STR);
    }
}
